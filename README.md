...
- We have test  [KAIST Complex Urban Dataset](https://sites.google.com/view/complex-urban-dataset) on "**new**" branch. We mainly made two changes:
  ...
  - We generate rosbag from the origin KAIST Complex Urban Dataset, and recover the `ring`and `time` fild of LiDAR pointcloud. You can using the ros package in [doc/kaise-help](./doc/kaist-help) to generate the rosbag.
...
- Test on  KAIST Complex Urban Dataset urban26 sequence.
  ...
  - Results of our modified code on urban26.bag:
    ...
    We can see that the trajectory has a large dirft, and the loop closure doesn't be detected successfully. This may be due to the reason that the LiDAR of the KAIST dataset is installed obliquely, resulting in too few valid pointclouds for registration.
